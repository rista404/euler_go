...
func horizontal() int {
	...
	for row := 0; row < size; row++ {
		for col := 0; col < size-seqn+1; col++ {
			p := product(nums[row][col : col+4])
			if p > max {
				max = p
			}
		}
...
func vertical() int {
	...
	for col := 0; col < size; col++ {
		for row := 0; row < size-seqn+1; row++ {
			digits := []int{nums[row][col], nums[row+1][col], nums[row+2][col], nums[row+3][col]}
			p := product(digits)
			if p > max {
				max = p
			}
...
func diagRight() int {
	...
	for row := 0; row < size-seqn+1; row++ {
		for col := 0; col < size-seqn+1; col++ {
			digits := []int{nums[row][col], nums[row+1][col+1], nums[row+2][col+2], nums[row+3][col+3]}
			p := product(digits)
			if p > max {
				max = p
			}
...
func main() {
	var max int
	res := []int{horizontal(), vertical(), diagLeft(), diagRight()}
	for _, r := range res {
		if r > max {
			max = r
		}
